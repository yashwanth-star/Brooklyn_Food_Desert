...
def create_map(gdf, year, coverage_ratio_col, rank_col, selected_rank, legend_name):
    ...
    folium.GeoJson(
        ...
        tooltip=folium.GeoJsonTooltip(
            ...
            aliases=['Census Tract Area', f'{year} Coverage Ratio', 'Rank'],
            localize=True
        )
    ).add_to(m)
        ...
        for _, row in gdf_filtered.iterrows():
            st.markdown(
                ...
                <div style="border:1px solid #ddd; border-radius: 10px; padding: 10px; margin: 10px 0; background-color: #f9f9f9;">
                    ...
                    <p><span style="color: #D32F2F;">{year} Coverage Ratio: </span>{row[coverage_ratio_col]}</p>
...
def main():
    ...
    if selection == "Home":
        ...
        with tabs[1]:
            ...
            rank_options = ['All'] + sorted(set([rank for rank in gdf_supermarkets[f'{year}_rank'] if pd.notna(rank)]), key=sort_ranks)
        ...
        with tabs[2]:
            ...
            rank_options = ['All'] + sorted(set([rank for rank in gdf_fast_food[f'{year}_rank'] if pd.notna(rank)]), key=sort_ranks)
